...
bootstrap-update-layer-config:
	cd $(TERRAFORM_GLOBAL_STATE_LAYER_DIR); \
	sed -i 's/$(STATE_S3_BUCKET_PLACEHOLDER)/$(STATE_S3_BUCKET_NAME)/g;s/$(STATE_DYNAMO_TABLE_PLACEHOLDER)/$(STATE_DYNAMO_TABLE_NAME)/g' $(TERRAFORM_MAIN_FILE) \
	sed -i 's/$(STATE_S3_BUCKET_PLACEHOLDER)/$(STATE_S3_BUCKET_NAME)/g;s/$(STATE_DYNAMO_TABLE_PLACEHOLDER)/$(STATE_DYNAMO_TABLE_NAME)/g;s/$(STATE_REGION_PLACEHOLDER)/$(STATE_REGION)/g' $(TERRAFORM_CONFIG_FILE) \
	sed -i 's/$(STATE_REGION_PLACEHOLDER)/$(STATE_REGION)/g' $(TERRAFORM_PROVIDER_FILE)


.PHONY: bootstrap-update-layer-config-undo
bootstrap-update-layer-config-undo:
	cd $(TERRAFORM_GLOBAL_STATE_LAYER_DIR); \
	sed -i 's/$(STATE_S3_BUCKET_NAME)/$(STATE_S3_BUCKET_PLACEHOLDER)/g;s/$(STATE_DYNAMO_TABLE_NAME)/$(STATE_DYNAMO_TABLE_PLACEHOLDER)/g' $(TERRAFORM_MAIN_FILE) \
	sed -i 's/$(STATE_S3_BUCKET_NAME)/$(STATE_S3_BUCKET_PLACEHOLDER)/g;s/$(STATE_DYNAMO_TABLE_NAME)/$(STATE_DYNAMO_TABLE_PLACEHOLDER)/g;s/$(STATE_REGION)/$(STATE_REGION_PLACEHOLDER)/g' $(TERRAFORM_CONFIG_FILE) \
	sed -i 's/$(STATE_REGION)/$(STATE_REGION_PLACEHOLDER)/g' $(TERRAFORM_PROVIDER_FILE)
